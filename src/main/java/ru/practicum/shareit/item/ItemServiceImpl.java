...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto getItemById(long id, long userId) {
        ...
        if (item.getOwner() != null && item.getOwner().getId() == userId) {
            ...
            bookingRepository.findFirstByItemIdAndStatusAndStartBeforeOrderByStartDesc(id, BookingStatus.APPROVED, now)
                    .ifPresent(booking -> itemDto.setLastBooking(booking.getStart()));

            bookingRepository.findFirstByItemIdAndStatusAndStartAfterOrderByStartAsc(id, BookingStatus.APPROVED, now)
    ...
    public CommentDto addComment(long id, long userId, CommentDto comment) {
        ...
        if (!hasCompletedBooking) {
            throw new BadRequestException("Пользователь не завершил бронирование этого товара");
        }
